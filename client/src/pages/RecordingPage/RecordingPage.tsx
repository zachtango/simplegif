...
function RecordingPage({
  ...
  async function load() {
    ...
    try {
      ...
      if(!stream)
        throw "Unable to retrieve screen stream"

      // Stream screen to video html
      if(!videoRef.current)
        throw "Video element not defined"
  ...
  return (
    <div className='w-full h-full flex justify-center'>
      <div className='my-[50px] h-4/6 min-h-[500px] flex flex-col items-center justify-center'>
        ...
        >
          <video className='max-h-full min-h-full' autoPlay playsInline ref={videoRef} />
        </div>
        ...
        {!isLoaded && (
          <button className='rounded-md bg-green-600 p-2 text-white' onClick={load}>

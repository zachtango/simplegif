...
function Video({
  ...
  const videoPlayHandler = () => {
    ...
    if (!(canvas && context && videoContext)) return;

    (function render() {
      const endTime =
        video.duration -
        (trimsRef.current[1] * video.duration) / TIMELINE_WIDTH;
      if (video.ended || video.currentTime > endTime) {
        video.currentTime =
          (trimsRef.current[0] * video.duration) / TIMELINE_WIDTH;
        video.play();
      }

      const crop = cropRef.current;

      if (!crop || video.paused) return;
  ...
  useEffect(() => {
    ...
    const mouseMoveHandler = (e: MouseEvent) => {
      ...
      const end = [
        (e.clientX - canvas.offsetLeft) * scale[0],
        (e.clientY - canvas.offsetTop) * scale[1],
      ];

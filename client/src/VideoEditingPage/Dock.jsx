...
function Dock({ ffmpeg, videoDuration, videoBlob, videoRef, onStopEditing }) {
  ...
  async function onSaveGif() {
    ...
    const MILLISECONDS_PER_PIXEL =
      VIDEO_DURATION_MILLISECONDS / VIDEO_EDITING_TIMELINE_WIDTH;

    const start = left * MILLISECONDS_PER_PIXEL;
    const end =
      (VIDEO_EDITING_TIMELINE_WIDTH - right - TRIM_SELECTOR_WIDTH_PX) *
    ...
    await ffmpeg.run(
      ...
      '-vf',
      RESOLUTION_TO_FFMPEG_ARG[resolution],
